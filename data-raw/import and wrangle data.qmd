...
system("dx download table-exporter_2025-03-17_09-32-33_data.csv", intern = TRUE) 

dataset <- read.csv(here::here("table-exporter_2025-03-17_09-32-33_data.csv"))
```
...
dx upload ascvd_col maybe not necessary
...
dataset_ascvd <- dataset %>% 
  ...
  left_join(ascvd_col) %>% 
  mutate(date_lost_to_follow_up = lubridate::as_date(date_lost_to_follow_up),
    ascvd = case_when(
    is.na(ascvd) ~ 0,
    ...
    ascvd_date = case_when(
      ascvd==0 ~ lubridate::as_date("2022-05-31"),
