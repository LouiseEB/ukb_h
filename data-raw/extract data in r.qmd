...
This is a guide to extract data from a dispensed dataset on DNAnexus.

Variable selection and creation of a list of field names is done in R and the data is then exported in TableExporter directly from Rstudio.
...
It is useful to programmatically identify all UKB-RAP field names associated with UK Biobank fields, especially if the field has many arrays or many fields are required.
...
filtered_dict <- datadict %>%
  filter(str_detect(title, "Age when at") |
          ...
          str_detect(title, "^Glycated haemoglobin \\(HbA1c\\) \\| Instance \\d+$") |
          ...
          str_detect(name, "p40000") | #date of death
